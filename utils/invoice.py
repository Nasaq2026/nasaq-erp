...
def generate_invoice_html(order_row, client_row):
    try:
        ...
        # بيانات العميل الضريبية
        comp_name = client_row[3] if client_row and len(client_row) > 3 and client_row[3] else client_name
        c_vat = client_row[4] if client_row and len(client_row) > 4 and client_row[4] else "---"
        c_cr = client_row[6] if client_row and len(client_row) > 6 and client_row[6] else "---"
        c_addr = client_row[5] if client_row and len(client_row) > 5 and client_row[5] else "المملكة العربية السعودية"
    ...
    <style>
        ...
        .inv-card {{ border: 2px solid #2980b9; padding: 30px; max-width: 850px; margin: auto; border-radius: 10px; background: #fff; }}
        ...
        <div class="inv-card">
            <div class="header">
                <div style="text-align: right;">
                    ...
                    <p style="margin:5px 0;">العنوان: تبوك - حي المروج</p>
            ...
            <div style="margin-top: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px; font-size: 14px;">
                <div style="border: 1px solid #eee; padding: 15px; border-radius: 5px;">
                    <p><b>العميل:</b> {comp_name}</p>
                    <p><b>الرقم الضريبي للعميل:</b> {c_vat}</p>
                    <p><b>السجل التجاري:</b> {c_cr}</p>
                    <p><b>العنوان الوطني:</b> {c_addr}</p>
                </div>
                <div style="text-align: left; border: 1px solid #eee; padding: 15px; border-radius: 5px;">
            ...
            <div style="margin-top: 20px; width: 45%; margin-right: auto;">
                <table style="width:100%; border-collapse: collapse; font-size: 16px;">
                    <tr style="background:#f9f9f9; border-bottom: 1px solid #ddd;"><td>الإجمالي شامل الضريبة:</td><td align="center"><b>{total:,.2f} ر.س</b></td></tr>
                    <tr style="border-bottom: 1px solid #ddd;"><td>المبلغ المدفوع:</td><td align="center">{paid:,.2f} ر.س</td></tr>
                    <tr style="color:red; font-weight:bold;"><td>المتبقي للمؤسسة:</td><td align="center">{total-paid:,.2f} ر.س</td></tr>
                </table>
            </div>
            <p style="text-align: center; margin-top: 40px; font-size: 11px; color: #888;">تعتبر هذه الفاتورة مستنداً رسمياً ضريبياً | طبعت عبر نظام نسق ERP</p>
        </div>
    </body>
    </html>
    """
