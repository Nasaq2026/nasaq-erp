...
# --- 3. ستايل نَسق (تصحيح المسافات وتمديد الأزرار) ---
...
<style>
    ...
    /* تنسيق أزرار القائمة الجانبية لتكون عريضة ومحترمة */
...
def load_system_pages():
    ...
    for key, (path, func) in modules.items():
        try:
            ...
            pages[key] = lambda *args, **kwargs: st.error(f"⚠️ الملف {key} مفقود في مجلد web_ui")
    return pages
...
def load_system_pages():
    ...
    for key, (path, func) in modules.items():
        try:
            ...
            pages[key] = lambda *args, **kwargs: st.error(f"⚠️ الملف {key} يحتاج فحص في مجلد web_ui")
    return pages
...
def main_portal():
    ...
    if page_key == "ai_assistant":
        ...
        role = st.session_state.role
        if role == "Designer":
            PAGES["designer"](conn, st.session_state.emp_name)
        elif role == "Technician":
            PAGES["tech"](conn, st.session_state.emp_name)
        elif role == "Installer":
            PAGES["installer"](conn, st.session_state.emp_name)
    elif page_key:
        PAGES[page_key](conn)
...
# --- 5. تحميل الصفحات (تشمل التسويق والواتساب) ---
def load_system_pages():
    pages = {}
    modules = {
        "dashboard": ("web_ui.dashboard", "render_dashboard"),
